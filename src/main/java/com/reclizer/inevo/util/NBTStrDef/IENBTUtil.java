package com.reclizer.inevo.util.NBTStrDef;

import com.reclizer.inevo.util.IENBT;
...
public class IENBTUtil {
    ...
    {
        SetInt(stack, IENBTDef.GUA_FREE_SOCKET, val);
    }

    public static boolean GetIsLearned(EntityPlayer player, int skillID)
    {
        int[] learnt = IENBT.getPlayerIdeallandIntArraySafe(player, IENBTDef.STARTER_KIT_VERSION_TAG);
    ...
    {
        int[] learnt = IENBT.getPlayerIdeallandIntArraySafe(player, IENBTDef.LEARNING_DONE);
        ...
        if (oldIndex >= 0)
        {
            ...
            if (val)
            {
                ...
                IENBT.setPlayerIdeallandTagSafe(player, IENBTDef.LEARNING_DONE, newLearnt);
            }else {
                return;
            }
        }
    ...
    {
        NBTTagCompound NBT = IENBTUtil.getNBT(stack);
        return new BlockPos(NBT.getDouble(IENBTDef.ANCHOR_X), NBT.getDouble(IENBTDef.ANCHOR_Y), NBT.getDouble(IENBTDef.ANCHOR_Z));
    }

    public static BlockPos getMarkedPos2(ItemStack stack)
    {
        NBTTagCompound NBT = IENBTUtil.getNBT(stack);
        return new BlockPos(NBT.getDouble(IENBTDef.ANCHOR_X_2), NBT.getDouble(IENBTDef.ANCHOR_Y_2), NBT.getDouble(IENBTDef.ANCHOR_Z_2));
    }

    public static void markPosToStack(ItemStack stack, BlockPos pos)
    {
        IENBTUtil.SetBoolean(stack, IENBTDef.ANCHOR_READY, true);
        IENBTUtil.SetDouble(stack, IENBTDef.ANCHOR_X, pos.getX());
        IENBTUtil.SetDouble(stack, IENBTDef.ANCHOR_Y, pos.getY());
        IENBTUtil.SetDouble(stack, IENBTDef.ANCHOR_Z, pos.getZ());
    }

    public static void markPosToStack2(ItemStack stack, BlockPos pos)
    {
        IENBTUtil.SetBoolean(stack, IENBTDef.ANCHOR_READY_2, true);
        IENBTUtil.SetDouble(stack, IENBTDef.ANCHOR_X_2, pos.getX());
        IENBTUtil.SetDouble(stack, IENBTDef.ANCHOR_Y_2, pos.getY());
        IENBTUtil.SetDouble(stack, IENBTDef.ANCHOR_Z_2, pos.getZ());
    }
}
