...
public class CommonFunctions {
    ...
    public static void LogPlayerAction(EntityLivingBase living, String action){
        IndustrialEvolved.Log(String.format("%s(%s): %s",living.getName(), living.getUniqueID(), action));
    }
    ...
    public static boolean TryConsumePlayerXP(EntityPlayer player, int XP)
    {
        ...
        while (costLeft > 0 && (player.experienceLevel > 0 || player.experience > 0))
        {
            if (player.experience > costLeft)
            {
                ...
                costLeft = 0;
                IndustrialEvolved.Log("A");
            }
            else {
                costLeft -= player.experience;
                IndustrialEvolved.Log("B");
                if (player.experienceLevel > 0)
                {
                    player.experienceLevel--;
                    player.experience = XPForLevel(player.experienceLevel);
                    IndustrialEvolved.Log(String.format("player.experience = %d", XPForLevel(player.experienceLevel)));
                }
            }
        }
        IndustrialEvolved.Log(String.format("Lv= %s, xp = %s", player.experienceLevel, player.experience));
        return true;
    }
